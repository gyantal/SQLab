    ...
    internal class SQLabCommonAspLogger : Microsoft.Extensions.Logging.ILogger, IDisposable
    {
        ...
        public static bool IsSendableToHealthMonitorForEmailing(Exception p_exception)
        {
            ...
            if (fullExceptionStr.IndexOf(@"Received an unexpected EOF or 0 bytes from the transport stream") != -1)
                isSendable = false;
            if (fullExceptionStr.IndexOf(@"Microsoft.AspNetCore.Server.Kestrel.BadHttpRequestException:") != -1)    // instead of all the separate conditions, omit all BadHttpExceptions.
                isSendable = false;
            ...
            //    isSendable = false;
            //if (fullExceptionStr.IndexOf(@"Microsoft.AspNetCore.Server.Kestrel.BadHttpRequestException: No ':' character found in header line") != -1)
            //    isSendable = false;
