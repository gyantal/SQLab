...
namespace VirtualBroker
{
    public partial class UberVxxStrategy : IBrokerStrategy
    {
        ...
        {
            StrongAssert.True((m_vxx != null) && (m_vxx.Length >= 101), Severity.ThrowException, "GetConnorForecast() cannot continue because VXXB historical data is not given. Throw exception. Notify administrators to investigate Log.");
            ...
            // if today %change = 0, try to short VXXB, because in general, 80% of the time, it is worth shorting VXXB than going long
            double forecast = 0;
            bool isFTRegime = m_probDailyFT > m_probDailyFTGoodFtRegimeThreshold;
            if (isFTRegime)   // FT regime
                forecast = (dailyPercentChange > 0) ? 1 : -1;        // FT regime: if %change = 0, we bet VXXB will go down (-1)
            else
                forecast = (dailyPercentChange >= 0) ? -1 : 1;// MR regime

            Utils.ConsoleWriteLine(ConsoleColor.Green, false, $"Connor: VXXB %Chg:{dailyPercentChange * 100.0:F2}%,ProbFT:{m_probDailyFT * 100.0}%,Regime:{((isFTRegime) ? "FT" : "MR")},Forecast:{forecast * 100}%");
            Utils.Logger.Info($"VXXB %Chg:{dailyPercentChange * 100.0:F2}%,ProbFT:{m_probDailyFT * 100.0}%,Regime:{((isFTRegime) ? "FT" : "MR")},Forecast:{forecast * 100}%");
            m_detailedReportSb.AppendLine($"<font color=\"#10ff10\">Connor: VXXB %Chg:{dailyPercentChange * 100.0:F2}%,ProbFT:{m_probDailyFT * 100.0}%,Regime:{((isFTRegime) ? "FT" : "MR")},Forecast:{forecast * 100}%</font>");
            return forecast;
        }
    }
}
