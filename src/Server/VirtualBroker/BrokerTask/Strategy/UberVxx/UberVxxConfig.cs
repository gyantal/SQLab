...
{
    //On skype Balazs said: VXXB StDev is 3.64% (from 2004), and sometimes he used 8% threshold(Zvalue= 2.2, eliminates 2.5% of the samples) or uses 11% threshold(Zvalue= 3.9, eliminates 1% of the stamples)
    ...
    //because if VXXB goes up by 15% that is obviously so exceptional(political decision) thing, and it is a random event comparing to the effect what we intend to meausure.
    ...
    public class UberVxxConfig
    {
        ...
        public double OutlierBasicZscore_Zscore = 3.0; // StDev of SPY is 1.1%, so this will eliminate 4.4% %Chg days, Balazs used 8% threshold for VXXB, which is about 8/3=2.7% threshold for SPY, which is about Zscore=2.5, because of Beta=3.

        // - the threshold for BullishEdge, BearishEdge (T-value, p-value) can be different. We are more willing to do Bearish LongVXXB, as a hedge of other portfolios, and because that is the less overcrowded trade.
