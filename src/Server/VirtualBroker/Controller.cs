            ...
            var harryLongTask = new SqTask()
            {
                ...
                Settings = new Dictionary<object, object>() {  //not necessary, because VBrokerTask can have local parameters inside itself
                    ...
                    { BrokerTaskSetting.Portfolios, new List<BrokerTaskPortfolio>()
                        ...
                        //        // Tickers = new string[] { "VXX", "VXZ", "TQQQ", "TMF", "USO", "UNG", "GAZ" }, AssetsWeights = new double[] { -0.20*cAgyLvrg, -0.10*cAgyLvrg, 0.30*cAgyLvrg, 0.90*cAgyLvrg, -0.28*cAgyLvrg, -0.23*cAgyLvrg, -0.25*cAgyLvrg}   // 2023-01-05: PTUP -10% tax problem. SCO (-2x) really bad. Tried to change SCO + USO to OIL, but 1. last 5years performance (short USO is much better than OIL), Short borrowing fee: USO: 4%, OIL: 30%, because OIL is not liquid (NAV is x100 more for USO). So, I keep USO for a while, as I don't do day trading. Taking the risk of future PTP problem, although USO has Qualified Notice now, so it is fine.
                        //        Tickers = new string[] { "VXX", "VXZ", "TQQQ", "TMF", "USO", "UNG" }, AssetsWeights = new double[] { -0.20*cAgyLvrg, -0.10*cAgyLvrg, 0.30*cAgyLvrg, 0.90*cAgyLvrg, -0.28*cAgyLvrg, -0.48*cAgyLvrg}   // 2023-03-30: Removing GAZ. Use UNG instead. Would be fine for stocks, but UNG options has so wide ask-bids it is untreadable. Better to take the risk of PTF of the very liquid UNG. So at the moment PTP danger is: USO, UNG. Have to monitor PTP. Fine.
                        ...
                        new BrokerTaskPortfolio() { Name = "! Harry Long2(Contango-Bond) harvester Live", HQUserID = HQUserID.drcharmat, IbGatewayUserToTrade = GatewayUser.CharmatSecondary,
                            ...
                            Param = new PortfolioParamHarryLong() { Tickers = new string[] {"VXX", "VXZ", "TQQQ", "TMV", "USO", "UNG" }, AssetsWeights = new double[] { -0.125*cDcLvrg, -0.10*cDcLvrg, 0.25*cDcLvrg, -0.85*cDcLvrg*cDcTmvLvrg, -0.26*cDcLvrg, -0.39*cDcLvrg }  } }  // 2023-04-26: Barclays discontinues OIL => USO, GAZ => UNG. Acting before the liquidity disappears.
