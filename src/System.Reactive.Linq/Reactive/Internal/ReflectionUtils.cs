...
#if HAS_WINRT
            // George: later I can check that addMethod.ReturnType is what.
            // this is how to remove later the eventHandlers:
            //"//Remove events one by one
            //foreach (Delegate del in eh.GetInvocationList())
            //info.GetRemoveMethod().Invoke(instance, new object[] { del });
            // but I don't want to work with that.
            // So, assume that the ReturnType of the method is NOT EventRegistrationToken, and I think the RX source code make it general, so it will work anyway.
            // so, let isWinRT = false; the target


            // I think, we can kill both the HAS_WINRT compile time, and _isWinRT runtime checks in the future.
            //But as a quick solution now, just comment out this part:
