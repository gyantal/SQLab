...
FROM (SELECT /*TopN*/ tt.* FROM (
    ...
    WHERE sq1.StockID=@id777 AND sq1.Date > @TsqEnd0777 -- handling overlap: BaseTicker (SVXY) is only used After the last date of SimulatedTicker (SVXY!Light0.5x.SQ)
