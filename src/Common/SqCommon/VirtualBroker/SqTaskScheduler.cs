...
namespace SqCommon
{
    ...
    public class SqTaskScheduler
    {
        ...
        internal void ScheduleTrigger(SqTrigger p_trigger, bool p_isMarketTradingDay, DateTime p_marketOpenTimeUtc, DateTime p_marketCloseTimeUtc)
        {
            ...
            Utils.Logger.Info("Task '" + p_trigger.SqTask.Name + "' next time: " + ((p_trigger.NextScheduleTimeUtc != null) ? ((DateTime)p_trigger.NextScheduleTimeUtc).ToString("dd'T'HH':'mm':'ss") : "null"));
        }

        private DateTime? CalcNextTriggerTime(SqTrigger p_trigger, bool p_isMarketTradingDay, DateTime p_marketOpenTimeUtc, DateTime p_marketCloseTimeUtc)
        {
